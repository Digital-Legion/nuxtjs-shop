<template>
  <div class="custom-input" :class="{error, 'custom-input--compact': compact}">
    <span class="common__label" v-if="label">{{ label }}</span>
    <div class="custom-input__input-wrapper">
      <textarea-autosize v-if="isTextarea" :disabled="disabled" :rows="textareaRows" :max-rows="5" class="custom-input__input" :value="value"
                         @input="onInput" :placeholder="placeholder"/>
      <input v-else-if="isPhone" :disabled="disabled" type="text" :value="value" @input="onInput" :placeholder="placeholder"
                  class="custom-input__input" v-mask="'+994 ## ### ## ##'" />
      <input v-else :type="filteredType" :disabled="disabled" :name="name" class="custom-input__input" :value="value" @input="onInput"
